...
SAML.prototype.processValidlySignedAssertion = function(xml, inResponseTo, callback) {
  ...
  .then(function(){
    ...
    if(self.options.issuer){
      var audienceRestriction = conditions.AudienceRestriction && conditions.AudienceRestriction[0];
      if (audienceRestriction && audienceRestriction.Audience && audienceRestriction.Audience.length > 0) {
        if (self.options.issuer != audienceRestriction.Audience) {
          msg = 'Audience "'+audienceRestriction.Audience+'" does not match ' + self.options.issuer;
          throw new Error(msg);
        }
      }
    }
